...
def compute_plasma(doses):
    ...
    idx = pd.date_range(start=doses[0][0], end=doses[-1][0] + timedelta(hours=12), freq=timedelta(minutes=30))
    ...
    for i in df.index[1:]:
        ...
        df.loc[i, 'unabsorbed'] += df.loc[i - 1, 'unabsorbed'] * np.exp2(-1 * stepsize_abs)
        df.loc[i, 'C'] += \
            df.loc[i - 1, 'C'] * np.exp2(-stepsize) \
            + df.loc[i - 1, 'unabsorbed'] * (1 - np.exp2(-stepsize_abs))
